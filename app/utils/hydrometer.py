...
def validate_inputs(temp_obs_f: float, lectura_api: float) -> None:
    ...
    if temp_obs_f < temp_min or temp_obs_f > temp_max:
        raise ValueError(f"Temperatura fuera del rango permitido: {temp_obs_f:.1f} °F")
    if lectura_api <= 0:
    ...
    if densidad < 610.6 or densidad > 1163.5:
        raise ValueError(f"Densidad calculada fuera de rango: {densidad:.1f} kg/m³")
...
def api_60f_crude(temp_obs_f: float, lectura_api: float) -> float:
    ...
    hyc = (
        1 - (0.00001278 * (temp_obs_f - 60)) - (0.0000000062 * ((temp_obs_f - 60) ** 2))
    )
    ...
    delta_tau = (
        ...
        + (
            a2
            + (a3 + (a4 + (a5 + (a6 + (a7 + (a8 * tau)) * tau) * tau) * tau) * tau)
            * tau
        )
        * tau
    ) * tau
    ...
    while True:
        ...
        b = (2.0 * k0 + k1 * densidad_m) / (k0 + (k1 + k2 * densidad_m) * densidad_m)
        de = densidad_m * (
            1.0
            + ((math.exp(a * (1.0 + 0.8 * a)) - 1.0) / (1.0 + a * (1.0 + 1.6 * a) * b))
        )
        ...
        d_pm = (2.0 * cpl * presion * fp * (7.9392 + 0.02326 * tf68)) / (densidad_m**2)
