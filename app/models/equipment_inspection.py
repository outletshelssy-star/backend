...
class EquipmentInspectionResponse(SQLModel, table=True):
    __tablename__ = "equipment_inspection_response"
    id: int | None = Field(default=None, primary_key=True)
    inspection_id: int = Field(foreign_key="equipment_inspection.id")
    inspection_item_id: int = Field(foreign_key="equipment_type_inspection_item.id")
...
class EquipmentInspectionCreate(SQLModel):
    ...
    notes: str | None = None
    responses: list[EquipmentInspectionResponseCreate] = Field(default_factory=list)
...
class EquipmentInspectionRead(SQLModel):
    ...
    notes: str | None
    is_ok: bool | None
    responses: list[EquipmentInspectionResponseRead] = Field(default_factory=list)
