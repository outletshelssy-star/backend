...
def _parse_monthly_readings_from_notes(
    ...
    if "alto equipo" not in lower_text and "medio equipo" not in lower_text and "bajo equipo" not in lower_text:
        return None
    ...
    if any(val is None for val in [alto_equipo, alto_patron, medio_equipo, medio_patron, bajo_equipo, bajo_patron]):
        return None
    ...
    return {
        ...
        "reading_under_test_unit": (alto_unit or "").lower() or None,
        "reference_reading_unit": (patron_unit or "").lower() or None,
    }
    ...
    if third is None:
        if not math.isclose(readings[0], readings[1], rel_tol=1e-9, abs_tol=1e-9):
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail=(
                    f"Cuando {label} tenga solo dos lecturas, deben ser iguales."
                ),
            )
        return readings
...
def create_equipment_verification(
    ...
    response_item_ids = [
        r.verification_item_id for r in payload.responses
    ]
    ...
    evaluated_responses: list[tuple[EquipmentVerificationResponseCreate, bool | None]] = []
    ...
    verified_at = _as_utc(payload.verified_at) if payload.verified_at else datetime.now(UTC)
    day_start = datetime(
        verified_at.year,
        verified_at.month,
        verified_at.day,
    ...
    applies_temperature_comparison_rule = (
        ...
        and _has_temperature_measure(session, equipment_type.id)
    )
    ...
    if applies_comparison_rule or applies_kf_verification_rule:
        ...
        if reference_equipment.id == equipment.id:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
        ...
        if applies_tape_comparison_rule:
            ...
            if not _has_length_measure(session, reference_type.id):
                raise HTTPException(
                    status_code=status.HTTP_400_BAD_REQUEST,
        ...
        if applies_hydrometer_comparison_rule:
            ...
            if is_monthly:
                ...
                try:
                    readings = [
                        (
                            "Alto",
                            float(payload.reading_under_test_high_value),
                            float(payload.reference_reading_high_value),
                        ),
                        (
                            "Medio",
                            float(payload.reading_under_test_mid_value),
                            float(payload.reference_reading_mid_value),
                        ),
                        (
                            "Bajo",
                            float(payload.reading_under_test_low_value),
                            float(payload.reference_reading_low_value),
                        ),
                    ]
                    ...
                    for label, under_val, ref_val in readings:
                        reading_under_test_c = _temperature_to_celsius(
                            under_val, payload.reading_under_test_unit
                        )
                        reference_reading_c = _temperature_to_celsius(
                            ref_val, payload.reference_reading_unit
                        )
                ...
                comparison_ok = all(delta_c <= max_delta_c for _, _, _, delta_c in diffs)
                if not comparison_ok:
                    comparison_message = (
                ...
                if not comparison_ok:
                    comparison_message = (
                        f"Difference between readings is {delta_c:.3f} C and exceeds maximum {max_delta_c:.3f} C."
                    )
            ...
            tape_under_unit = payload.reading_under_test_unit
            tape_ref_unit = payload.reference_reading_unit
            ...
            length_spec = _get_length_measure_spec(session, equipment.id)
    ...
    if existing_same_day:
        if not replace_existing:
            raise HTTPException(
                ...
                detail="Ya se realizo una verificacion hoy. Deseas reemplazarla?",
            )
        if existing_same_day.id is not None:
            ...
            session.exec(
                delete(EquipmentVerification).where(
                    EquipmentVerification.id == existing_same_day.id
                )
            )
            session.commit()
    ...
    if applies_comparison_rule or applies_kf_verification_rule:
        if applies_temperature_comparison_rule and is_monthly:
            try:
                ...
                ref_unit = payload.reference_reading_unit
                under_high = float(payload.reading_under_test_high_value)
                under_mid = float(payload.reading_under_test_mid_value)
                under_low = float(payload.reading_under_test_low_value)
                ref_high = float(payload.reference_reading_high_value)
                ref_mid = float(payload.reference_reading_mid_value)
                ref_low = float(payload.reference_reading_low_value)
                delta_high = abs(
            ...
            comparison_note = (
                f"Comparacion mensual {equipment_type.name} | "
            ...
            if use_unit_values:
                reading_under_test_label = (
                    f"{payload.reading_under_test_value} {payload.reading_under_test_unit}"
                )
                reference_reading_label = (
                    f"{payload.reference_reading_value} {payload.reference_reading_unit}"
                )
            else:
                reading_under_test_label = f"{payload.reading_under_test_f} F"
                reference_reading_label = f"{payload.reference_reading_f} F"
            comparison_note = (
                f"Comparacion {equipment_type.name} | "
                f"Patron ID: {payload.reference_equipment_id} | "
            ...
            comparison_note = (
                f"Comparacion cinta {equipment_type.name} | "
                f"Patron ID: {payload.reference_equipment_id} | "
    ...
    if verification_ok is False:
        ...
        session.commit()
        message = comparison_message or "Verification failed. Equipment status set to needs_review."
...
def update_equipment_verification(
    ...
    verified_at = _as_utc(payload.verified_at) if payload.verified_at else _as_utc(verification.verified_at)
    day_start = datetime(
        verified_at.year,
        verified_at.month,
        verified_at.day,
    ...
    existing_same_day = session.exec(
        select(EquipmentVerification).where(
            ...
            EquipmentVerification.verified_at >= day_start,
            EquipmentVerification.verified_at < day_end,
            EquipmentVerification.id != verification.id,
        )
    ).first()
    if existing_same_day:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Ya existe una verificacion para esta fecha.",
        )
