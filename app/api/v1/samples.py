...
def _validate_regular_user_analyzed_at(
    user: User,
    analyzed_at: datetime | None,
    existing_analyzed_at: datetime | None = None,
) -> None:
    if user.user_type != UserType.user or analyzed_at is None:
        return
    normalized_analyzed_at = _as_utc(analyzed_at)
    normalized_existing = (
        _as_utc(existing_analyzed_at) if existing_analyzed_at is not None else None
    )
    if normalized_existing is not None and normalized_analyzed_at == normalized_existing:
        return
    now = datetime.now(UTC)
    earliest_allowed = now - timedelta(hours=72)
    if normalized_analyzed_at < earliest_allowed or normalized_analyzed_at > now:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail=(
                "Los usuarios solo pueden seleccionar una fecha de analisis "
                "dentro de las ultimas 72 horas."
            ),
        )
...
def create_sample(
    ...
    # Validate identifier before acquiring the row lock
    ...
    _validate_regular_user_analyzed_at(current_user, payload.analyzed_at)
    ...
    sample = Sample(
        ...
        analyzed_at=_as_utc(payload.analyzed_at) if payload.analyzed_at else None,
        volume=payload.volume,
        retention_days=payload.retention_days,
    )
    session.add(sample)
    session.flush()  # get sample.id without committing
    sample_id = sample.id
    ...
    for analysis in payload.analyses:
        ...
        session.add(row)
    session.commit()  # single commit: terminal seq + sample + analyses
    ...
    return SampleRead(
        ...
        retention_days=sample.retention_days,
        disposed_at=_as_utc(sample.disposed_at) if sample.disposed_at else None,
        disposed_by_user_id=sample.disposed_by_user_id,
        analyses=[
...
def list_samples(
    ...
    all_analyses = session.exec(
        select(SampleAnalysis).where(SampleAnalysis.sample_id.in_(sample_ids))  # type: ignore[union-attr]
    ).all()
    analyses_by_sample: dict[int, list[SampleAnalysis]] = {}
    for a in all_analyses:
        analyses_by_sample.setdefault(a.sample_id, []).append(a)

    items: list[SampleRead] = []
    for sample in samples:
        if sample.id is None:
            continue
        analyses = analyses_by_sample.get(sample.id, [])
        items.append(
            SampleRead(
                id=sample.id,
                terminal_id=sample.terminal_id,
...
def update_sample(
    ...
    is_disposal_only = list(update_data.keys()) == ["disposed_at"]
    update_reason = str(payload.update_reason or "").strip()
    if not is_disposal_only and datetime.now(UTC) - _as_utc(sample.created_at) >= timedelta(hours=24):
    ...
    if payload.analyzed_at is not None:
        _validate_regular_user_analyzed_at(
            current_user,
            payload.analyzed_at,
            sample.analyzed_at,
        )
    ...
    if payload.volume is not None:
        sample.volume = payload.volume
    if payload.retention_days is not None:
        sample.retention_days = payload.retention_days
    if payload.disposed_at is not None:
        sample.disposed_at = _as_utc(payload.disposed_at)
        if current_user.id is not None:
            sample.disposed_by_user_id = current_user.id
