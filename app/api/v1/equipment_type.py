...
def _load_equipment_type_prefetch_data(
    session: Session,
    equipment_types: list[EquipmentType],
    include_set: set[str],
) -> dict[str, Any]:
    equipment_type_ids = [
        equipment_type.id for equipment_type in equipment_types if equipment_type.id is not None
    ]
    measures_by_equipment_type_id: dict[int, list[EquipmentMeasureType]] = {}
    max_errors_by_equipment_type_id: dict[int, list[EquipmentTypeMaxErrorRead]] = {}
    users_by_id: dict[int, User] = {}
    inspection_items_by_equipment_type_id: dict[
        int, list[EquipmentTypeInspectionItemRead]
    ] = {}
    verification_types_by_equipment_type_id: dict[
        int, list[EquipmentTypeVerificationRead]
    ] = {}

    if not equipment_type_ids:
        return {
            "measures_by_equipment_type_id": measures_by_equipment_type_id,
            "max_errors_by_equipment_type_id": max_errors_by_equipment_type_id,
            "users_by_id": users_by_id,
            "inspection_items_by_equipment_type_id": inspection_items_by_equipment_type_id,
            "verification_types_by_equipment_type_id": verification_types_by_equipment_type_id,
        }

    measures = session.exec(
        select(EquipmentTypeMeasure).where(
            EquipmentTypeMeasure.equipment_type_id.in_(equipment_type_ids)  # type: ignore[attr-defined]
        )
    ).all()
    for measure in measures:
        measures_by_equipment_type_id.setdefault(
            measure.equipment_type_id,
            [],
        ).append(measure.measure)

    max_errors = session.exec(
        select(EquipmentTypeMaxError).where(
            EquipmentTypeMaxError.equipment_type_id.in_(equipment_type_ids)  # type: ignore[attr-defined]
        )
    ).all()
    for max_error in max_errors:
        max_errors_by_equipment_type_id.setdefault(
            max_error.equipment_type_id,
            [],
        ).append(EquipmentTypeMaxErrorRead(**max_error.model_dump()))

    if "creator" in include_set:
        creator_ids = {equipment_type.created_by_user_id for equipment_type in equipment_types}
        if creator_ids:
            users_by_id = {
                user.id: user
    ...
    if "inspection_items" in include_set:
        inspection_items = session.exec(
            ...
            .where(
                EquipmentTypeInspectionItem.equipment_type_id.in_(equipment_type_ids)  # type: ignore[attr-defined]
            )
            .order_by(
                EquipmentTypeInspectionItem.equipment_type_id,  # type: ignore[arg-type]
                EquipmentTypeInspectionItem.order,  # type: ignore[arg-type]
            )
        ).all()
        for inspection_item in inspection_items:
            inspection_items_by_equipment_type_id.setdefault(
                inspection_item.equipment_type_id,
                [],
            ).append(EquipmentTypeInspectionItemRead(**inspection_item.model_dump()))

    if "verification_types" in include_set:
        verification_types = session.exec(
            select(EquipmentTypeVerification)
            .where(
                EquipmentTypeVerification.equipment_type_id.in_(equipment_type_ids)  # type: ignore[attr-defined]
            )
            .order_by(
                EquipmentTypeVerification.equipment_type_id,  # type: ignore[arg-type]
                EquipmentTypeVerification.order,  # type: ignore[arg-type]
            )
        ).all()
        for verification_type in verification_types:
            verification_types_by_equipment_type_id.setdefault(
                verification_type.equipment_type_id,
                [],
            ).append(EquipmentTypeVerificationRead(**verification_type.model_dump()))
...
def _build_equipment_type_read_with_includes(
    ...
    *,
    ...
    response.measures = prefetch_data["measures_by_equipment_type_id"].get(
        equipment_type_id,
        [],
    )
    response.max_errors = prefetch_data["max_errors_by_equipment_type_id"].get(
        equipment_type_id,
        [],
    )

    if "creator" in include_set:
        creator = prefetch_data["users_by_id"].get(equipment_type.created_by_user_id)
        if creator is not None:
            response.creator = _to_user_ref(creator)
    if "inspection_items" in include_set:
        response.inspection_items = prefetch_data[
            "inspection_items_by_equipment_type_id"
        ].get(equipment_type_id, [])
    if "verification_types" in include_set:
        response.verification_types = prefetch_data[
            "verification_types_by_equipment_type_id"
        ].get(equipment_type_id, [])
...
def create_equipment_type(
    ...
    for max_error in equipment_type_in.max_errors:
        ...
        elif max_error.measure == EquipmentMeasureType.volume:
            normalized_value = _normalize_volume(
                max_error.max_error_value, max_error.unit
            )
...
def list_equipment_types(
    ...
    prefetch_data = _load_equipment_type_prefetch_data(
        session,
        equipment_types,
        include_set,
    )
    items = [
        _build_equipment_type_read_with_includes(
            equipment_type,
...
def get_equipment_type(
    ...
    return _build_equipment_type_read_with_includes(
        equipment_type,
        include_set=include_set,
        prefetch_data=prefetch_data,
    )
