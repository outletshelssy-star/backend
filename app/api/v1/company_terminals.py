...
from app.models.enums import ProductType, UserType
...
from app.models.terminal_product_type import TerminalProduct
...
def _get_terminal_has_samples_map(
    session: Session,
    terminal_ids: list[int],
) -> dict[int, bool]:
    unique_terminal_ids = sorted({terminal_id for terminal_id in terminal_ids})
    if not unique_terminal_ids:
        return {}
    terminal_ids_with_samples = session.exec(
        select(Sample.terminal_id)
        .where(Sample.terminal_id.in_(unique_terminal_ids))  # type: ignore[union-attr]
        .group_by(Sample.terminal_id)
    ).all()
    return {
        int(terminal_id): True
        for terminal_id in terminal_ids_with_samples
        if terminal_id is not None
    }


def _assert_terminal_access(
    session: Session,
    current_user: User,
    terminal_id: int,
) -> None:
    ...
    allowed_terminal_ids = session.exec(
        select(UserTerminal.terminal_id).where(UserTerminal.user_id == current_user.id)
    ).all()
...
def _list_terminal_products(session: Session, terminal_id: int) -> list[TerminalProduct]:
    return session.exec(
        select(TerminalProduct).where(TerminalProduct.terminal_id == terminal_id).order_by(TerminalProduct.id)  # type: ignore[arg-type]
    ).all()


def _default_product_name(product_type: ProductType) -> str:
    if product_type == ProductType.crudo:
        return "Crudo"
    if product_type == ProductType.gasolina:
        return "Gasolina"
    if product_type == ProductType.diesel:
        return "Diesel"
    return str(product_type).strip().capitalize()


class TerminalProductPayload(SQLModel):
    name: str
...
class TerminalProductsPayload(SQLModel):
    products: list[TerminalProductPayload]


class TerminalProductTypesPayload(SQLModel):
    product_types: list[ProductType]


def _replace_terminal_products(
    session: Session,
    terminal_id: int,
    products: list[TerminalProductPayload],
) -> list[TerminalProduct]:
    session.exec(
        delete(TerminalProduct).where(TerminalProduct.terminal_id == terminal_id)  # type: ignore[arg-type]
    )
    seen: set[tuple[str, ProductType]] = set()
    for product in products:
        dedupe_key = (product.name.casefold(), product.product_type)
    ...
    session.commit()
    return _list_terminal_products(session, terminal_id)
...
def list_company_terminals(
    session: Session = Depends(get_session),
    current_user: User = Depends(require_role(UserType.visitor, UserType.user, UserType.admin, UserType.superadmin)),
    include: str | None = Query(
        default=None,
        description=("Relaciones a incluir, separadas por coma: `block`, `owner_company`, `admin_company`, `creator`."),
    ),
    ...
    if owner_company_id is not None:
        statement = statement.where(CompanyTerminal.owner_company_id == owner_company_id)
    if current_user.user_type != UserType.superadmin:
        allowed_terminal_ids = session.exec(
            select(UserTerminal.terminal_id).where(UserTerminal.user_id == current_user.id)
        ).all()
    ...
    terminal_ids = [terminal.id for terminal in terminals if terminal.id is not None]
    has_samples_by_terminal_id = _get_terminal_has_samples_map(session, terminal_ids)
    ...
    if not include_set:
        return CompanyTerminalListResponse(
            items=[
                CompanyTerminalReadWithIncludes(
                    **terminal.model_dump(),
                    has_samples=has_samples_by_terminal_id.get(
                        terminal.id or -1,
                        False,
                    ),
                )
    ...
    for terminal in terminals:
        ...
        results.append(
            CompanyTerminalReadWithIncludes(
                **terminal.model_dump(),
                has_samples=has_samples_by_terminal_id.get(terminal.id or -1, False),
                block=block,
...
def get_company_terminal(
    ...
    if terminal.id is not None:
        has_samples = _get_terminal_has_samples_map(session, [terminal.id]).get(
            terminal.id,
            False,
        )
    include_set = {item.strip() for item in (include or "").split(",") if item.strip()}
...
def delete_company_terminal(
    ...
    terminal_data = CompanyTerminalReadWithIncludes(
        **terminal.model_dump(),
        has_samples=has_samples,
    )
    session.exec(
    ...
    session.exec(
        delete(TerminalProduct).where(
            TerminalProduct.terminal_id == terminal_id  # type: ignore[arg-type]
        )
    )
...
@router.get("/{terminal_id}/products")
def get_terminal_products(
    terminal_id: int,
    session: Session = Depends(get_session),
    current_user: User = Depends(require_role(UserType.visitor, UserType.user, UserType.admin, UserType.superadmin)),
) -> list[TerminalProductPayload]:
    terminal = session.get(CompanyTerminal, terminal_id)
    if not terminal:
        raise HTTPException(status_code=404, detail="Terminal no encontrado.")
    _assert_terminal_access(session, current_user, terminal_id)
    items = _list_terminal_products(session, terminal_id)
    return [TerminalProductPayload(name=item.name, product_type=item.product_type) for item in items]


@router.put("/{terminal_id}/products")
def update_terminal_products(
    terminal_id: int,
    payload: TerminalProductsPayload,
    session: Session = Depends(get_session),
    current_user: User = Depends(require_role(UserType.admin, UserType.superadmin)),
) -> list[TerminalProductPayload]:
    terminal = session.get(CompanyTerminal, terminal_id)
    if not terminal:
        raise HTTPException(status_code=404, detail="Terminal no encontrado.")
    _assert_terminal_access(session, current_user, terminal_id)
    items = _replace_terminal_products(session, terminal_id, payload.products)
    return [TerminalProductPayload(name=item.name, product_type=item.product_type) for item in items]


@router.get("/{terminal_id}/product-types")
def get_terminal_product_types(
    terminal_id: int,
    session: Session = Depends(get_session),
    current_user: User = Depends(require_role(UserType.visitor, UserType.user, UserType.admin, UserType.superadmin)),
) -> list[str]:
...
@router.put("/{terminal_id}/product-types")
def update_terminal_product_types(
    terminal_id: int,
    payload: TerminalProductTypesPayload,
    session: Session = Depends(get_session),
    current_user: User = Depends(require_role(UserType.admin, UserType.superadmin)),
) -> list[str]:
    terminal = session.get(CompanyTerminal, terminal_id)
    ...
    _assert_terminal_access(session, current_user, terminal_id)
    products: list[TerminalProductPayload] = []
    seen: set[ProductType] = set()
    for pt in payload.product_types:
        if pt in seen:
            continue
        seen.add(pt)
        products.append(
            TerminalProductPayload(
                name=_default_product_name(pt),
                product_type=pt,
            )
        )
    items = _replace_terminal_products(session, terminal_id, products)
    product_types: list[str] = []
    for item in items:
        pt = str(item.product_type)
