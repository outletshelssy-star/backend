...
def _normalize_volume(value: float, unit: str) -> float:
    unit_key = unit.strip().lower()
    if unit_key in {"ml", "milliliter", "milliliters"}:
        return value
    if unit_key in {"l", "liter", "liters"}:
        return value * 1000.0
    raise HTTPException(
        status_code=status.HTTP_400_BAD_REQUEST,
        detail="Unsupported volume unit",
    )
...
def _load_equipment_prefetch_data(
    session: Session,
    equipment_items: list[Equipment],
    include_set: set[str],
) -> dict[str, Any]:
    equipment_ids = [equipment.id for equipment in equipment_items if equipment.id is not None]
    measure_specs_by_equipment_id: dict[int, list[EquipmentMeasureSpecRead]] = {}
    component_serials_by_equipment_id: dict[int, list[EquipmentComponentSerialRead]] = {}
    equipment_types_by_id: dict[int, EquipmentType] = {}
    companies_by_id: dict[int, Company] = {}
    terminals_by_id: dict[int, CompanyTerminal] = {}
    users_by_id: dict[int, User] = {}
    inspections_by_equipment_id: dict[int, list[EquipmentInspectionRead]] = {}
    verifications_by_equipment_id: dict[int, list[EquipmentVerificationRead]] = {}
    calibrations_by_equipment_id: dict[int, list[EquipmentCalibrationRead]] = {}

    if not equipment_ids:
        return {
            "measure_specs_by_equipment_id": measure_specs_by_equipment_id,
            "component_serials_by_equipment_id": component_serials_by_equipment_id,
            "equipment_types_by_id": equipment_types_by_id,
            "companies_by_id": companies_by_id,
            "terminals_by_id": terminals_by_id,
            "users_by_id": users_by_id,
            "inspections_by_equipment_id": inspections_by_equipment_id,
            "verifications_by_equipment_id": verifications_by_equipment_id,
            "calibrations_by_equipment_id": calibrations_by_equipment_id,
        }

    measure_specs = session.exec(
        select(EquipmentMeasureSpec).where(
            EquipmentMeasureSpec.equipment_id.in_(equipment_ids)  # type: ignore[attr-defined]
        )
    ).all()
    for spec in measure_specs:
        measure_specs_by_equipment_id.setdefault(spec.equipment_id, []).append(
            EquipmentMeasureSpecRead(**spec.model_dump())
        )

    component_serials = session.exec(
        select(EquipmentComponentSerial).where(
            EquipmentComponentSerial.equipment_id.in_(equipment_ids)  # type: ignore[attr-defined]
        )
    ).all()
    for component in component_serials:
        component_serials_by_equipment_id.setdefault(component.equipment_id, []).append(
            EquipmentComponentSerialRead(**component.model_dump())
        )

    if "equipment_type" in include_set:
        equipment_type_ids = {equipment.equipment_type_id for equipment in equipment_items}
        if equipment_type_ids:
            equipment_types_by_id = {
                equipment_type.id: equipment_type
                for equipment_type in session.exec(
                    select(EquipmentType).where(
                        EquipmentType.id.in_(equipment_type_ids)  # type: ignore[attr-defined]
                    )
                ).all()
                if equipment_type.id is not None
            }

    if "owner_company" in include_set:
        company_ids = {equipment.owner_company_id for equipment in equipment_items}
        if company_ids:
            companies_by_id = {
                company.id: company
                for company in session.exec(
                    select(Company).where(
                        Company.id.in_(company_ids)  # type: ignore[attr-defined]
                    )
                ).all()
    ...
    if "terminal" in include_set:
        terminal_ids = {equipment.terminal_id for equipment in equipment_items}
        if terminal_ids:
            terminals_by_id = {
                terminal.id: terminal
                for terminal in session.exec(
                    select(CompanyTerminal).where(
                        CompanyTerminal.id.in_(terminal_ids)  # type: ignore[attr-defined]
                    )
                ).all()
    ...
    if "creator" in include_set:
        creator_ids = {equipment.created_by_user_id for equipment in equipment_items}
        if creator_ids:
            users_by_id = {
                user.id: user
                for user in session.exec(
                    select(User).where(User.id.in_(creator_ids))  # type: ignore[attr-defined]
                ).all()
    ...
    if "inspections" in include_set:
        inspections = session.exec(
            select(EquipmentInspection).where(
                EquipmentInspection.equipment_id.in_(equipment_ids)  # type: ignore[attr-defined]
            )
        ).all()
        inspection_ids = [inspection.id for inspection in inspections if inspection.id is not None]
        inspection_responses_by_inspection_id: dict[
            int, list[EquipmentInspectionResponseRead]
        ] = {}
        if inspection_ids:
            inspection_responses = session.exec(
                select(EquipmentInspectionResponse).where(
                    EquipmentInspectionResponse.inspection_id.in_(inspection_ids)  # type: ignore[attr-defined]
                )
            ).all()
            for response in inspection_responses:
                if response.id is None:
                    continue
                inspection_responses_by_inspection_id.setdefault(
                    response.inspection_id,
                    [],
                ).append(EquipmentInspectionResponseRead(**response.model_dump()))
        for inspection in inspections:
            if inspection.id is None:
                continue
            inspections_by_equipment_id.setdefault(inspection.equipment_id, []).append(
                EquipmentInspectionRead(
                    **inspection.model_dump(),
                    responses=inspection_responses_by_inspection_id.get(
                        inspection.id,
                        [],
                    ),
                )
    ...
    if "verifications" in include_set:
        verifications = session.exec(
            select(EquipmentVerification).where(
                EquipmentVerification.equipment_id.in_(equipment_ids)  # type: ignore[attr-defined]
            )
        ).all()
        verification_ids = [
            verification.id for verification in verifications if verification.id is not None
        ]
        verification_responses_by_verification_id: dict[
            int, list[EquipmentVerificationResponseRead]
        ] = {}
        if verification_ids:
            verification_responses = session.exec(
                select(EquipmentVerificationResponse).where(
                    EquipmentVerificationResponse.verification_id.in_(verification_ids)  # type: ignore[attr-defined]
                )
            ).all()
            for response in verification_responses:
                if response.id is None:
                    continue
                verification_responses_by_verification_id.setdefault(
                    response.verification_id,
                    [],
                ).append(
                    EquipmentVerificationResponseRead(**response.model_dump())
                )
        for verification in verifications:
            if verification.id is None:
                continue
            verification_read = EquipmentVerificationRead(
                **verification.model_dump(),
                responses=verification_responses_by_verification_id.get(
                    verification.id,
                    [],
                ),
            )
            ...
            verifications_by_equipment_id.setdefault(
                verification.equipment_id,
                [],
            ).append(verification_read)

    if "calibrations" in include_set:
        calibrations = session.exec(
            select(EquipmentCalibration).where(
                EquipmentCalibration.equipment_id.in_(equipment_ids)  # type: ignore[attr-defined]
            )
        ).all()
        calibration_ids = [
            calibration.id for calibration in calibrations if calibration.id is not None
        ]
        calibration_results_by_calibration_id: dict[
            int, list[EquipmentCalibrationResultRead]
        ] = {}
        if calibration_ids:
            calibration_results = session.exec(
                select(EquipmentCalibrationResult).where(
                    EquipmentCalibrationResult.calibration_id.in_(calibration_ids)  # type: ignore[attr-defined]
                )
            ).all()
            for result in calibration_results:
                if result.id is None:
                    continue
                calibration_results_by_calibration_id.setdefault(
                    result.calibration_id,
                    [],
                ).append(EquipmentCalibrationResultRead(**result.model_dump()))
        for calibration in calibrations:
            if calibration.id is None:
                continue
            calibrations_by_equipment_id.setdefault(calibration.equipment_id, []).append(
                EquipmentCalibrationRead(
                    **calibration.model_dump(),
                    results=calibration_results_by_calibration_id.get(
                        calibration.id,
                        [],
                    ),
                )
...
def _build_equipment_read_with_includes(
    equipment: Equipment,
    *,
    include_set: set[str],
    prefetch_data: dict[str, Any],
) -> EquipmentReadWithIncludes:
    equipment_id = _require_id(equipment.id, "Equipment")
    response = EquipmentReadWithIncludes.model_validate(
        equipment,
        from_attributes=True,
    )
    response.measure_specs = prefetch_data["measure_specs_by_equipment_id"].get(
        equipment_id,
        [],
    )
    response.component_serials = prefetch_data["component_serials_by_equipment_id"].get(
        equipment_id,
        [],
    )

    if "equipment_type" in include_set:
        equipment_type = prefetch_data["equipment_types_by_id"].get(
            equipment.equipment_type_id
        )
        if equipment_type is not None:
            response.equipment_type = _to_equipment_type_ref(equipment_type)
    if "owner_company" in include_set:
        owner_company = prefetch_data["companies_by_id"].get(equipment.owner_company_id)
        if owner_company is not None:
            response.owner_company = _to_company_ref(owner_company)
    if "terminal" in include_set:
        terminal = prefetch_data["terminals_by_id"].get(equipment.terminal_id)
        if terminal is not None:
            response.terminal = _to_terminal_ref(terminal)
    if "creator" in include_set:
        creator = prefetch_data["users_by_id"].get(equipment.created_by_user_id)
        if creator is not None:
            response.creator = _to_user_ref(creator)
    if "inspections" in include_set:
        response.inspections = prefetch_data["inspections_by_equipment_id"].get(
            equipment_id,
            [],
        )
    if "verifications" in include_set:
        response.verifications = prefetch_data["verifications_by_equipment_id"].get(
            equipment_id,
            [],
        )
    if "calibrations" in include_set:
        response.calibrations = prefetch_data["calibrations_by_equipment_id"].get(
            equipment_id,
            [],
        )
...
def create_equipment(
    ...
    inicial de tipo, terminal y estado del equipo.
    """
    ...
    for spec in equipment_in.measure_specs:
        ...
        elif spec.measure == EquipmentMeasureType.volume:
            min_value = _normalize_volume(spec.min_value, spec.min_unit)
            max_value = _normalize_volume(spec.max_value, spec.max_unit)
            resolution = (
                _normalize_volume(spec.resolution, spec.resolution_unit)
                if spec.resolution is not None
                else None
            )
...
def list_equipment(
    ...
    prefetch_data = _load_equipment_prefetch_data(
        session,
        equipment_items,
        include_set,
    )
    items = [
        _build_equipment_read_with_includes(
            equipment,
...
def get_equipment(
    ...
    return _build_equipment_read_with_includes(
        equipment,
        include_set=include_set,
        prefetch_data=prefetch_data,
    )
...
def update_equipment(
    ...
    componentes. Registra cambios de tipo, terminal o estado en el historial.
    """
    if current_user.id is None:
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
def list_equipment_history(
    ...
    """
    Lista el historial combinado de cambios de tipo, terminal y estado.
    ...
    for status_entry in status_history:
        if status_entry.changed_by_user_id:
            user_ids.add(status_entry.changed_by_user_id)
    ...
    for status_entry in status_history:
        items.append(
            EquipmentHistoryEntry(
                id=f"status-{status_entry.id}",
                kind="status",
                equipment_type_id=None,
                terminal_id=None,
                status=status_entry.status,
                started_at=status_entry.started_at,
                ended_at=status_entry.ended_at,
                changed_by_user_id=status_entry.changed_by_user_id,
                changed_by_user_name=user_name_by_id.get(status_entry.changed_by_user_id),
            )
        )
