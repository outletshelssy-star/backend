...
def _process_inspection_responses(
    items_by_id: dict,
    required_ids: set,
    responses: list[EquipmentInspectionResponseCreate],
) -> list[tuple[EquipmentInspectionResponseCreate, bool | None]]:
    response_item_ids = [r.inspection_item_id for r in responses]
    ...
    evaluated: list[tuple[EquipmentInspectionResponseCreate, bool | None]] = []
    for response in responses:
        ...
        evaluated.append((response, _evaluate_response(item, response)))
    return evaluated
...
def create_equipment_inspection(
    ...
    evaluated_responses = _process_inspection_responses(
        items_by_id, required_ids, payload.responses
    )
...
def delete_equipment_inspection(
    inspection_id: int,
    session: Session = Depends(get_session),
    current_user: User = Depends(require_role(UserType.admin, UserType.superadmin)),
) -> None:
    ...
    session.exec(
        delete(EquipmentInspectionResponse).where(
            EquipmentInspectionResponse.inspection_id == inspection_id  # type: ignore[arg-type]
        )
    )
