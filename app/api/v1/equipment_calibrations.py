...
def _infer_measure_from_unit(unit: str | None) -> EquipmentMeasureType | None:
    ...
    if unit_key in {"ml", "l", "milliliter", "milliliters", "liter", "liters"}:
        return EquipmentMeasureType.volume
...
def _normalize_uncertainty_value(
    ...
    if measure == EquipmentMeasureType.volume:
        unit_key = (unit or "ml").strip().lower()
        ...
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="Unsupported volume unit for uncertainty",
        )
...
def create_equipment_calibration(
    ...
    calibration_day = calibrated_at.date()
    existing = session.exec(
        select(EquipmentCalibration).where(
            EquipmentCalibration.equipment_id == equipment_id,
            func.date(EquipmentCalibration.calibrated_at) == calibration_day,
        )
    ).first()
    if existing:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail=f"Ya existe una calibración para este equipo en la fecha {calibration_day.isoformat()}.",
        )
...
def list_equipment_calibrations(
    ...
    calibration_ids = [c.id for c in calibrations if c.id is not None]
    all_results = session.exec(
        select(EquipmentCalibrationResult).where(
            EquipmentCalibrationResult.calibration_id.in_(calibration_ids)  # type: ignore[union-attr]
        )
    ).all()
    results_by_calibration: dict[int, list[EquipmentCalibrationResult]] = {}
    for r in all_results:
        results_by_calibration.setdefault(r.calibration_id, []).append(r)
        ...
        items=[
            EquipmentCalibrationRead(
                **c.model_dump(),
                results=[
                    EquipmentCalibrationResultRead.model_validate(r, from_attributes=True)
                    for r in results_by_calibration.get(c.id or 0, [])
                ],
            )
...
def delete_equipment_calibration(
    calibration_id: int,
    session: Session = Depends(get_session),
    current_user: User = Depends(
    ...
    """
    Elimina una calibración por ID (incluyendo sus resultados).
